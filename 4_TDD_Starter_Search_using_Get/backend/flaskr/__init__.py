...
def create_app(test_config=None):
    ...
    # Method Search -- Get based on search_term
    @app.route('/books/<string:search_term>', methods=['GET'])
    def search_book(search_term):
        search = "%{}%".format(search_term)
        searchResult = Book.query.filter(Book.title.ilike(search)).all()
        current_books = paginate_books(request, searchResult)
        ...
        return jsonify(
            {
                ...
                "books": current_books,
                "total_books": len(Book.query.all()),
                "search_count":len(current_books)
            }
        )
